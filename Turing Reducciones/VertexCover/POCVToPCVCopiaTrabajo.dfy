...
//hasta que todas las aristas estén cubiertas
 ...
 while (g.1 != {})//Paramos cuando todas las aristas estan cubiertas
  ...
  invariant I <= graph.0 - vertex <= I + g.0
 ...
 { assume vertex !={} ;
  ...
  if kg' == kg - 1 // kg' == kg - 1
   ...
   // y se eliminan todas las aristas cubiertas por dicho vértice
    ...
    else {assert optimalValueVertexCover(g,kg);}
  ...
  //assert (set edge:Edge, node:Node | edge in graph.1 && node in I && node in edge :: edge) == graph.1;
