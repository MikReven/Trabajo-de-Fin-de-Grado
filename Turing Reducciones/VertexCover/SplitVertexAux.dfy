...
//Any optimal Vertex Cover of the split graph an be transformed to a Vertex Cover of the Original Graph without changing its size
...
ensures exists I: set<Node> :: I <= graph.0 && isVertexCover(I, graph) && |I| <= |I'|
{
    if I' <= graph.0 {
...
{
  forall n | n in neighborsOf(graph, v) 
  ensures n in I'  
  {
    assume false;
    var m :| m in graph'.0 - graph.0 && m != n && {n, m} in graph'.1;
    //The edge {n, m} must be covered by I', so n or m must belong to it, but m cannot belong to it since it does not belong to graph.0, so it must be n that belongs
  }
